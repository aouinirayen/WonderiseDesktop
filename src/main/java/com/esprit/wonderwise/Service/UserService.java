...
public class UserService {
    /**
     * Login using Google account. Returns User if email exists, else null.
     * Optionally, you can link googleId to the user table for future logins.
     */
    public User loginWithGoogle(String email, String googleId) {
    ...
    // Generate and store reset token, return the token if successful, else null
    public String generateAndStoreResetToken(String email) {
        String token = generateToken();
        boolean ok = storeToken(email, token);
    ...
    public User login(String email, String password) {
        ...
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                boolean isBCrypt = storedPassword != null && storedPassword.matches("^\\$2[aby]\\$.{56}$");
                ...
                if (isBCrypt) {
                    ...
                    // If matches, upgrade password to BCrypt
                    if (passwordMatches) {
                        String newHash = BCrypt.hashpw(password, BCrypt.gensalt());
                        // Update DB with new hash
                        try (PreparedStatement psUpdate = cnx.prepareStatement("UPDATE users SET password=? WHERE email=?")) {
