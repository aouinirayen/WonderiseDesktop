...
public class UsersBackController {
    ...
    private VBox createUserCard(User user) {
        ...
        if (user.getStatus() != null && user.getStatus().equalsIgnoreCase("Pending")) {
            ...
            unblockBtn.setStyle("-fx-background-color: #27ae60; -fx-text-fill: white; -fx-background-radius: 8; -fx-font-size: 12px; -fx-padding: 4 18;");
            unblockBtn.setOnAction(e -> handleUnblockUser(user));
            btns.getChildren().addAll(editBtn, delBtn, unblockBtn);
        } else {
            Button banBtn = new Button("Ban");
            banBtn.setStyle("-fx-background-color: #f39c12; -fx-text-fill: white; -fx-background-radius: 8; -fx-font-size: 12px; -fx-padding: 4 18;");
            banBtn.setOnAction(e -> handleBanUser(user));
            btns.getChildren().addAll(editBtn, delBtn, banBtn);
        }
    ...
    private void handleUnblockUser(User user) {
        ...
        alert.setHeaderText("Are you sure you want to unblock this user?");
        alert.setContentText("Username: " + user.getUsername() + "\nEmail: " + user.getEmail());
        ButtonType okButton = new ButtonType("Unblock", ButtonBar.ButtonData.OK_DONE);
        ButtonType cancelButton = new ButtonType("Cancel", ButtonBar.ButtonData.CANCEL_CLOSE);
        alert.getButtonTypes().setAll(okButton, cancelButton);
        alert.showAndWait().ifPresent(type -> {
            if (type == okButton) {
                ...
                if (updated) {
                    refreshUsers();
                    // Send unblock email asynchronously
                    javafx.concurrent.Task<Void> emailTask = new javafx.concurrent.Task<>() {
                        @Override
                        protected Void call() {
                            try {
                                com.esprit.wonderwise.Utils.MailUtil.sendHtmlMail(
                                    user.getEmail(),
                                    "Your Wonderwise Account Has Been Unblocked",
                                    com.esprit.wonderwise.Utils.MailUtil.unblockUserTemplate(user.getUsername())
                                );
                    ...
                    error.setTitle("Error");
                    error.setHeaderText(null);
                    error.setContentText("Failed to unblock user. Please try again.");
                    error.showAndWait();
                }
            }
        });
    ...
    private void handleBanUser(User user) {
        ...
        alert.setContentText("Username: " + user.getUsername() + "\nEmail: " + user.getEmail());
        ButtonType okButton = new ButtonType("Ban", ButtonBar.ButtonData.OK_DONE);
        ButtonType cancelButton = new ButtonType("Cancel", ButtonBar.ButtonData.CANCEL_CLOSE);
        alert.getButtonTypes().setAll(okButton, cancelButton);
        alert.showAndWait().ifPresent(type -> {
            if (type == okButton) {
                ...
                if (updated) {
                    refreshUsers();
                    // Send block email asynchronously
                    javafx.concurrent.Task<Void> emailTask = new javafx.concurrent.Task<>() {
                        @Override
                        protected Void call() {
                            try {
                                com.esprit.wonderwise.Utils.MailUtil.sendHtmlMail(
                                    user.getEmail(),
                                    "Your Wonderwise Account Has Been Blocked",
                                    com.esprit.wonderwise.Utils.MailUtil.blockUserTemplate(user.getUsername())
                                );
                            } catch (jakarta.mail.MessagingException e) {
                                e.printStackTrace();
                            }
                    ...
                    error.setTitle("Error");
                    error.setHeaderText(null);
                    error.setContentText("Failed to ban user. Please try again.");
                    error.showAndWait();
                }
            }
        });
